...
class MoveGroupPythonInterface(object):
    ...
    def get_points_from_img(self, topic):
        ...
        marker_size = 100  # 10 см
        obj_points = np.array([
            [-marker_size/2.0, marker_size/2.0, 0],  # нижний левый
            [ marker_size/2.0, marker_size/2.0, 0],  # нижний правый
            [ marker_size/2.0,  -marker_size/2.0, 0],  # верхний правый
            [-marker_size/2.0,  -marker_size/2.0, 0]   # верхний левый
        ], dtype=np.float32)

        K = np.array([
            [2554.414833, 0.000000, 681.885900],
            [0.000000, 2557.461333, 597.316448],
            [0.000000, 0.000000, 1.000000]
        ], dtype=np.float32)

        dist = np.array(
            [-0.063212, 0.479756, 0.007614, 0.003480, 0.0],
            dtype=np.float32
        )
        success, rvec_m2c, tvec_m2c = cv2.solvePnP(obj_points, img_points, K, dist, flags=cv2.SOLVEPNP_IPPE_SQUARE)
        # print(f"{success}, {rvec_m2c}, {tvec_m2c}")
        ...
        T_m1_2_c = np.eye(4)
        T_m1_2_c[:3, :3] = R_m2c
        T_m1_2_c[:3, 3] = tvec_m2c.flatten()

        tvec_r2m = np.array([-162.0, -274.0, 2.5], dtype=np.float32)
        T_m1_2_r = np.eye(4)
        T_m1_2_r[:3, 3] = tvec_r2m


        T_c_2_r = T_m1_2_r @ np.linalg.inv(T_m1_2_c)


        img_points2 = corners[1][0]
        marker_size = 40  
        obj_points = np.array([
            [-marker_size/2.0, marker_size/2.0, 0],  # нижний левый
            [ marker_size/2.0, marker_size/2.0, 0],  # нижний правый
        ...
        success, rvec_m12c, tvec_m12c = cv2.solvePnP(obj_points, img_points2, K, dist, flags=cv2.SOLVEPNP_IPPE_SQUARE)

        R_m2c, _ = cv2.Rodrigues(rvec_m12c)  
        T_m2_2_c = np.eye(4)
        T_m2_2_c[:3, :3] = R_m2c
        T_m2_2_c[:3, 3] = tvec_m12c.flatten()

        T_m2_2_r = T_c_2_r @ T_m2_2_c

        # --- Выход в МЕТРАХ (MoveIt!) ---
        x = T_m2_2_r[0, 3] / 1000.0
        y = T_m2_2_r[1, 3] / 1000.0
